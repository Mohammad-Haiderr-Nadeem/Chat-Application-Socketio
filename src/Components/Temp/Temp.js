...
function Temp ({ name, setName }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('http://localhost:8000/generateHashForge', {
        text
      });
      setGeneratedHash(res.data.hash.data);
      setText('');
    } catch (err) {
  ...
  const handlePasswordValidity = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('http://localhost:8000/validateHashForge', { hash, myPassword });
      console.log('res: ', res);
      if (res.data.msg === 'SUCCESSFUL') {
        alert('VALID!' + ' ' + res.data.password);
        setMyPassword('');
      }
    } catch (err) {
