...
import styles from './Temp4.module.css';
import NavbarComponent from '../Navbar/Navbar';
import axios from 'axios';

function Temp4 ({ name, setName }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('http://localhost:8000/generateHashBcrypt', {
        text
      });
  ...
  const handlePasswordValidity = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('http://localhost:8000/validateHashBcrypt', { hash, myPassword });
