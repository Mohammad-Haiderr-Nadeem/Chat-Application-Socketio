...
function Temp2 ({ name, setName }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('http://localhost:8000/generateHashCrypto', {
        text
      });
  ...
  const handlePasswordValidity = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('http://localhost:8000/validateHashCrypto', { hash, myPassword });
